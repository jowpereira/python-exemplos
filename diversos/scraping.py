...
class Validation(object):
    ...
    y = Validation()
    dataIni = dt.time(hour=4, minute=10)
    dataFim = dt.time(hour=23, minute=35)

    if (y.validation(dataIni, dataFim, 'Low', 'CHF')):
    ...
    def __init__(self, url="https://www.energyexch.com/calendar?day=today"):
        self.__url = url
    ...
    def validation(self, initDate, endDate, impact, currency):
        for index, row in self.__prepareDate().iterrows():
            x = row.Time_Eastern
            if x == 'Tentative' or x == 'All Day' or str(x).find('Day') is not -1:
                continue
            if self.__process_time(dt.time(hour=x.hour, minute=x.minute), dt.time(hour=initDate.hour, minute=initDate.minute), dt.time(hour=endDate.hour, minute=endDate.minute)) and self.__noticias[['Impact']].values[index] == impact and str(self.__noticias['Currency']).find(currency) is not -1:
                return True
    ...
    def __prepareDate(self):
        ...
        for index, row in horasNoticia.iterrows():
            ...
            if isinstance(m2, str):
                m2 = m2.replace(u'\xa0', '')
                if m2 == 'Tentative' or m2 == 'All Day' or m2.find('Day') is not -1:
                    continue
                m2 = m2.replace("p", " p")
                m2 = m2.replace("a", " a")
    ...
    def __scraper(self):
        ...
        for item in table:
            ...
            try:
                ...
                pass
            dict["Actual"] = item.find_all("td", {"class": "calendar__actual"})[
                0].text      # Actual Value
            dict["Forecast"] = item.find_all("td", {"class": "calendar__forecast"})[
                0].text  # forecasted Value
            dict["Previous"] = item.find_all("td", {"class": "calendar__previous"})[
                0].text  # Previous
            forcal.append(dict)
...
if __name__ == "__main__":
    ...
    y = Validation("https://www.forexfactory.com/calendar?day=today")
    dataIni = dt.time(hour=4, minute=10)
    dataFim = dt.time(hour=23, minute=35)

    if (y.validation(dataIni, dataFim, 'Low', 'AUD')):
        print('não negocia, está dentro do horario de noticias e impacto X')
    else:
        print('negocia, está fora do horario de noticias')